...
class Excel:
    ...
    def write_exel(self, path, beautiful=(True, 'max'), now_date=False):
        """
    ...
    def get_df_bytes(self):
        ...
        for column in df:
            column_width = len(column) + 5
            col_idx = df.columns.get_loc(column)
            writer.sheets['Результат'].set_column(col_idx, col_idx, column_width)
        writer.save()
    ...
    @staticmethod
    def drop_duplicates(read_path, subset, save_path=''):
        if not save_path:
            save_path = read_path
...
def append_to_excel(post, log, ex: Excel, extra):
    ...
    if log.location and hasattr(post, 'town'):
        ex.add_key_value('Местоположение товара', post.town)
    if hasattr(post, 'phone'):
        if post.phone:
            exists = extra.get('exists')
            ex.add_key_value('Контакт', post.phone)
            if exists:
                ex.add_key_value('WhatsApp', 'Есть')
            else:
                ex.add_key_value('WhatsApp', 'Нет')
    ...
    if log.created and hasattr(post, 'created'):
        ...
        ex.add_key_value('Дата создания объявления', created)
    if log.seller_reg and hasattr(post, 'seller_reg'):
        ...
        ex.add_key_value('Дата регистрации продавца', seller_reg)
    ...
    if hasattr(post, 'seller_rating'):
        ex.add_key_value('Рейтинг продавца', post.seller_rating)


def create_excel(posts: List[dict], log, extra: dict) -> Excel:
    ex = Excel()
    ...
    ex.set_data(data)
    for post in posts:
        if log.title and post.get('title'):
            ex.add_key_value('Товар', post.get('title'))
        if log.description and post.get('description'):
            ex.add_key_value('Описание', post.get('description'))
        if log.price:
            currency = extra.get('currency', '')
            ex.add_key_value('Цена', f'{post.get("price")} {currency}')
        if log.location and post.get("town"):
            ex.add_key_value('Местоположение товара', post.get("town"))
        if post.get("phone"):
            exists = extra.get('exists')
            ex.add_key_value('Контакт', post.get("phone"))
            if exists:
                ex.add_key_value('WhatsApp', 'Есть')
            else:
                ex.add_key_value('WhatsApp', 'Нет')
        if log.views and post.get("views"):
            ex.add_key_value('Просмотров', post.get("views"))
        if log.created and post.get("created"):
            ex.add_key_value('Дата создания объявления', post.get("created"))
        if log.seller_reg and post.get("seller_reg"):
            ex.add_key_value('Дата регистрации продавца', post.get("seller_reg"))
        if log.posts_count and post.get("seller_posts"):
            ex.add_key_value('Обьявлений продавца', post.get("seller_posts"))
        ex.add_key_value('Ссылка на товар', post.get("link"))
        if post.get("photo_url"):
            ex.add_key_value('Ссылка на фото', post.get("photo_url"))
        if log.seller_name and post.get("seller_name"):
            ex.add_key_value('Имя продавца', post.get("seller_name"))
        if post.get("seller_rating"):
            ex.add_key_value('Рейтинг продавца', post.get("seller_rating"))
